...
void launchIsawaKernel(unsigned int numBlocks, unsigned int threadsPerBlock, float3* positions, float3* velocities, float dt) {
    ...
    IsawaKernel<<<numBlocks, threads>>>(positions, velocities, dt);
...
void launchChenKernel(unsigned int numBlocks, unsigned int threadsPerBlock, float3* positions, float3* velocities, float dt) {
    ...
    chenKernel<<<numBlocks, threads>>>(positions, velocities, dt);
...
void launchLorentzKernel(unsigned int numBlocks, unsigned int threadsPerBlock, float3* positions, float3* velocities, float dt) {
    ...
    lorenzKernel<<<numBlocks, threads>>>(positions, velocities, dt);    
...
// __global__ void gravityKernel(float3* positions, float3* d_velocity, float dt) {
//     int tid = threadIdx.x;
//     int col_offset = blockDim.x * blockDim.y * blockIdx.x;
//     int row_offset = gridDim.x * blockIdx.y * blockDim.x * blockDim.y + blockDim.x * threadIdx.y;
//     int i = tid + col_offset + row_offset;
...
// void launchInitKernel(unsigned int numBlocks, unsigned int threadsPerBlock, float3* positions, char * type) {
//     cudaError_t cudaStatus;
//     unsigned int sqrtThreads = (unsigned int)sqrt(threadsPerBlock);
//     static curandState *states = NULL;
//     cudaGetErrorString(cudaMalloc((void **)&states, sizeof(curandState) *
